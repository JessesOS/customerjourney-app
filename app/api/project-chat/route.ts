...
function cleanAnswer(answer: string, terse = false) {
  const normalized = answer
    .replace(/^tab\s+\d+\s+/i, "")
    .replace(
      /^here is a summary draft structured for your new document\.\s+you can copy and paste this content directly into your file\.\s*/i,
      ""
    )
    .replace(/^answer generated by\s+\[?read\.ai\]?:?\s*/i, "")
