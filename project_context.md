...
## Completed: Stage voice guide (2026-08-05)

Imprint-inspired narrator: a breathing orb + caption bubble under each stage blurb —
tap to hear a short (~10s) per-stage narration. No autoplay (browser policy + taste).
Speaker toggle in the topbar beside the theme dots; off = feature hidden, persisted per
device (localStorage `pjVoiceOff`).

- Scripts: `lib/stageGuide.ts` (single source of truth — 5 Scale + 4 Respond stages).
- Audio: `public/portal/voice/{scale|respond}-{stageId}.m4a`, regenerated by
  `scripts/generate-voice.py` (edge-tts, free Microsoft neural voice
  **en-AU-NatashaNeural**). Jesse preferred William's steadier delivery but wants a
  female voice — for the eventual ElevenLabs upgrade, brief is "William's steadiness,
  female". Swapping voice = replace the m4a files, zero code changes.
- Orb animation: `pjBreathe` keyframes in globals.css, colours ride `--pj-act` so it
  re-themes (terracotta warm / petrol cool).
- Next layer discussed, not built: "Ask AI" chips (Imprint teases example questions
  during onboarding), possible avatar/video later. Imprint reference frames analysed
  2026-08-05 — the "hologram" feel was just orb + captions + voice, no character.
